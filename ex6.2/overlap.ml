...
let overlap_point_rectangle f1 f2 =
    ...
    (loc_x -. float_of_int(x) /. 2.0 <= f1.loc_x) && (loc_x +. float_of_int(x) /. 2.0 >= f1.loc_x) && (loc_y -. float_of_int(y) /. 2.0 <= f1.loc_y) && (loc_y +. float_of_int(y) /. 2.0 >= f1.loc_y);;
...
let overlap_circle_rectangle f1 f2 =
  ...
  let {loc_x = rx; loc_y = ry; body = Rectangle (x, y)} = f2 in
  let (c1, c2, c3, c4) = four_points f2 in
...
let overlap_rectangle_rectangle f1 f2 =
  ...
  if x1 * y1 < x2 * y2 then
    let (c1, c2, c3, c4) = four_points f1 in
...
let rectangle_of_square f =
  match f.body with
      Square x' ->
	{loc_x = f.loc_x; loc_y = f.loc_y; body = Rectangle(x', x')}
    | _ ->
      f;;
